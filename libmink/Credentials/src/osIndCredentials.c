...
//NOTE: If the CBOR is externally imported, then we allocate the CBOR buf size
//up to max size, and if it is constructed internally by the lib,
//then the actual constructed CBOR size buf is allocated.
#define CBOR_IMPORT_MAX_LEN 12288
#define CBOR_CONSTRUCT_LEN 32
...
enum {
    ...
    eEEUUID = ('e' << 16 | 'i' << 8 | 'd'),
    eVmUUID = ('v' << 8 | 'm'),
    eEEDID = ('e' << 24 | 'd' << 16 | 'i' << 8 | 'd'),
    eEEHash = ('e' << 24 | 'h' << 16 | 's' << 8 | 'h'),
    eEEDomain = ('e' << 24 | 'd' << 16 | 'm' << 8 | 'n'),
    eEEDistName = ('e' << 16 | 'd' << 8 | 'n'),
    eEEVersion = ('e' << 24 | 'v' << 16 | 'e' << 8 | 'r'),
...
enum {
    ...
    // env
    indOSid,
    indEEUUID,  // input from hypervisor
    indEEDID,   // generated
    indEEHash,
    indEEDomain,
    indEEDistName,  // generated
    indEEVersion,
    endOfIndex,
    ...
    lastEnvCredIndex = indEEVersion,

} OSind_FLAGS;

// used to convert index to key (string)
static const char *indToEKeyChar[] = {"n",    "apid", "ahsh", "adbg", "aver",
                                      "ccrd", "cdid",  "dn",   "dmn",  "uid",
                                      "pid",  "secl", "perm", "os",   "eid",
                                      "edid", "ehsh", "edmn",  "edn", "ever"};
...
/**
 * Description: converts bin to hex.
 * In:          buf: the buf of bin
 *              buf_sz: size of buf
 *              str: hex buf
 *              str_sz: size of str
 *
 * Out:         string length
 *
 * @note This does not enforce null termination of the output buffer (str).
 */
size_t bin_to_hex(const uint8_t* buf, size_t buf_sz,
                  char* str, size_t str_sz)
{
    ...
    for (offset = 0; offset < buf_sz; offset++) {
        str[2U * offset] = hex_str[(buf[offset] & (uint8_t)0xF0) >> 4];
        str[2U * offset + 1U] = hex_str[buf[offset] & (uint8_t)0x0F];
    }
 ...
 *              strBufSize: size of uuidCharArray
 *
 *
 * Out:         uuidStrBuf: uuid string (hexadecimal)
 */
static int32_t convert_uuid_to_string(uuid_t *uuid, char *uuidStrBuf,
                                      size_t strBufSize)
{
    size_t idx = 0;

    if (!uuid || !uuidStrBuf || UUID_STRING_LENGTH + 1 > strBufSize) {
        return Object_ERROR;
    }

    idx += bin_to_hex((uint8_t*)uuid, sizeof(uuid_t), uuidStrBuf, strBufSize);
    uuidStrBuf[idx] = '\0';
 ...
 */
static int32_t _generate_CBOR(ProcessCred *me, uint32_t uid)
{
    ...
    QCBOREncode_Init(&ECtx, me->legacyCBOR, CBOR_CONSTRUCT_LEN);
    QCBOREncode_OpenMap(&ECtx);

    //The CBOR cred is retained for backward compatibility with LA,
    //so we use the same getuid as LA here to get the uid,
    //rather than the uuid format uid.
    QCBOREncode_AddInt64ToMapN(&ECtx, AttrUid, uid);

    T_GUARD(gettimeofday(&tv, NULL));
    millis = tv.tv_sec * 1000 + tv.tv_usec / 1000;
    QCBOREncode_AddInt64ToMapN(&ECtx, AttrSystemTime, millis);

    QCBOREncode_CloseMap(&ECtx);
    return QCBOREncode_Finish2(&ECtx, &Enc);
                               ...
                               size_t *valueLenOut)
{
    ...
    *valueLenOut = 0;
    ...
    switch (_strKey((const char *)name, nameLen)) {
        ...
        case eVmUUID:  // EEUUID
        {
            if (BIT_TST(me->flags, indEEUUID)) {
                data = &me->vmUUID;
                data_size = sizeof(me->vmUUID);
            }
            break;
        }
        case eEEDID:  // EEDistinguishedId
        {
            if (BIT_TST(me->flags, indEEDID)) {
                data = &me->vmDID;
        ...
        case eEEHash:  // EEHash
        {
            if (BIT_TST(me->flags, indEEHash)) {
                data = &me->vmHash;
                data_size = sizeof(me->vmHash);
            }
            break;
        }
        case eEEDomain:  // EEDomain
        {
            if (BIT_TST(me->flags, indEEDomain)) {
                data = me->vmDomain;
                data_size = strlen(me->vmDomain) + 1;
            }
            break;
        }
        case eEEDistName:  // EEDistinguishedName
        {
            if (BIT_TST(me->flags, indEEDistName)) {
                data = me->vmDistUUID;
                data_size = strlen(me->vmDistUUID) + 1;
            }
            break;
        }
        case eEEVersion:  // EEVersion
        {
            if (BIT_TST(me->flags, indEEVersion)) {
                data = &me->vmVersion;
...
int32_t OSIndCredentials_newProcessCred(uint64_t permissions,
                                        ...
                                        uint8_t *legacyCBOR, size_t legacyCBORLen,
                                        ...
                                        uint32_t uid, Object *outCredentials)
{
    ...
    if (legacyCBOR) {
        // system input
        T_CHECK(legacyCBORLen <= CBOR_IMPORT_MAX_LEN)
        me->legacyCBOR = HEAP_ZALLOC_ARRAY(uint8_t, legacyCBORLen);
        T_CHECK_ERR(me->legacyCBOR, Object_ERROR_MEM);
        memscpy(me->legacyCBOR, legacyCBORLen, legacyCBOR, legacyCBORLen);
        me->CBORLen = legacyCBORLen;
        BIT_SET(me->flags, indLegacyCBOR);
    } else {
#ifdef LEGACY_CBOR_CRED
        me->legacyCBOR = HEAP_ZALLOC_ARRAY(uint8_t, CBOR_CONSTRUCT_LEN);
        T_CHECK_ERR(me->legacyCBOR, Object_ERROR_MEM);
        T_GUARD(_generate_CBOR(me, uid));
        me->CBORLen = CBOR_CONSTRUCT_LEN;
        BIT_SET(me->flags, indLegacyCBOR);
#endif
    }
                                    ...
                                    uint32_t vmVersion, Object *outCredentials)
{
    ...
    if (vmHash) {
        ...
        BIT_SET(me->flags, indEEHash);
    }
    T_GUARD(_setCharField(&me->flags, indEEDomain, &me->vmDomain, vmDomain));
    me->vmVersion = vmVersion;

    // set flags now because _generate checks if certain fields exist using
    // flags
    BIT_SET(me->flags, indOSid);
    BIT_SET(me->flags, indEEUUID);
    BIT_SET(me->flags, indEEDomain);
    BIT_SET(me->flags, indEEVersion);

    // generate values that need to be generated
    T_GUARD(_generateDistinguishedName(
        &me->flags, indEEDistName, (char **)&me->vmDistUUID, indEEDomain,
        (char **)&me->vmDomain, indEEUUID, (uint8_t *)&me->vmUUID.val));
    T_GUARD(_generateDistinguishedId(
        &me->flags, me->vmDistUUID, indEEDID, (DistId **)&me->vmDID,
        indEEDomain, &me->vmDomain, indEEUUID, (uint8_t *)&me->vmUUID));
