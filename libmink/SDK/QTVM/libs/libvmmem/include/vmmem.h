...
public:
     ...
     */
    int LendDmabuf(int dma_buf_fd, const VmPerm& perms, int64_t* memparcel_hdl = nullptr);

    /*
     * Should be used instead of LendDmaBuf if the local VM
     * needs to be able to access the dmabuf.
     */
    int ShareDmabuf(int dma_buf_fd, const VmPerm& perms, int64_t* memparcel_hdl = nullptr);
     ...
     */
    int RetrieveDmabuf(VmHandle owner, const VmPerm& perms, int64_t memparcel_hdl);
     ...
     */
    int RemoteAllocDmabuf(uint64_t size, const VmPerm& perms, const std::string& src_dma_heap_name,
                          const std::string& dst_dma_heap_name);

private:
    VmMem();
    int PopulateAcl(struct acl_entry* acl, uint32_t nr_acl_entries, const VmPerm& perm);
    int PopulateRemoteAllocData(union RemoteAllocData* data, __u32* mem_type,
                                const std::string& dma_heap_name, const std::string& ion_heap_name);
    int QueryIonHeaps();
    int DetectBackend();
    /* Ioctl Versions */
    int LendDmabufIoctl(int fd, const VmPerm& dst_perms, int64_t* memparcel_hdl);
    int ShareDmabufIoctl(int fd, const VmPerm& dst_perms, int64_t* memparcel_hdl);
    int RetrieveDmabufIoctl(VmHandle owner, const VmPerm& perms, int64_t memparcel_hdl);
