...
static int32_t ShareMemory_QueryExistingMemparcelHandle(int32_t dmaBufFd, int64_t *outMPHandle)
{
  ...
  if (ret) {
    LOG_ERR("ioctl(MEM_BUF_IOC_GET_MEMPARCEL_HDL) with dmaBufFd=%d failed, errno=%d!\n",
            dmaBufFd, errno);
    ret = Object_ERROR;
    goto exit;
  }
  ...
  *outMPHandle = getMemParcelHdlArg.memparcel_hdl;
  LOG_MSG("ioctl(MEM_BUF_IOC_GET_MEMPARCEL_HDL) with dmaBufFd=%d passed.\n", dmaBufFd);
  ret = Object_OK;
                                       ...
                                       char *destVMName, int64_t *outMPHandle)
{
  ...
  if (!(ITAccessPermissions_removeSelfAccess & specialRules)) {
    ret = vm_osal_shareDmabufHandle(vmMem, dmaBufFd, handleArr, permArr, nr, outMPHandle);
    LOG_PERF("conf = %p, dmaFd = %d, mpH = %u%09u, getShareWmH \n", &conf, dmaBufFd,
              UINT64_HIGH(*outMPHandle), UINT64_LOW(*outMPHandle));
  } else {
    ret = vm_osal_lendDmabufHandle(vmMem, dmaBufFd, handleArr, permArr, nr, outMPHandle);
    LOG_PERF("conf = %p, dmaFd = %d, mpH = %u%09u, getLendWmH \n", &conf, dmaBufFd,
             UINT64_HIGH(*outMPHandle), UINT64_LOW(*outMPHandle));
  }

  if (ret < 0) {
    LOG_ERR("%s of dmaBufFd=%d failed with ret=%d.\n",
            !(ITAccessPermissions_removeSelfAccess & specialRules)
            ? "ShareDmabufHandle" : "LendDmabufHandle",
            dmaBufFd, ret);
    ret = Object_ERROR;
    goto exit;
  } else {
    LOG_MSG("%s of dmaBufFd=%d passed with ret=%d.\n",
            !(ITAccessPermissions_removeSelfAccess & specialRules)
            ? "ShareDmabufHandle" : "LendDmabufHandle",
            dmaBufFd, ret);
    ret = Object_OK;
                             ...
                             Object *objOut)
{
  ...
  if (Object_isERROR(MSMem_new_remote(outFd, confRules, memparcelHandle, lockPtr, objOut))) {
    LOG_ERR("creating new MSMem of shared/lent memBuf failed!!\n");
    ret = Object_ERROR_MEM;
    goto exit;
  }

  LOG_PERF("getMSMem = %p, dmaFd = %d, confRule = %p, dmaSize = %u%09u, returnGetMSMem \n",
           objOut, outFd, confRules, UINT64_HIGH(vm_osal_getDMAFdSize(outFd)),
           UINT64_LOW(vm_osal_getDMAFdSize(outFd)));
...
int32_t ShareMemory_ReclaimMemBuf(int32_t fd, int64_t memparcelHandle)
{
  ...
  if (ret < 0) {
    LOG_ERR("Warning: ReclaimDmaBuf fail but this operation wont leak the memory.\n");
    ret = Object_ERROR;
  } else {
    ret = Object_OK;
  }
