...
static int ThreadPool_createThread(ThreadPool *me)
{
  int ret = -1;

  for (int i = 0; i < THREADPOOL_MAX_THREADS; ++i) {
    if (me->aThreads[i] == 0) {
      ret = vm_osal_thread_create(&me->aThreads[i], &thread_entrypoint, me, NULL);
      if (0 == ret) {
        ...
        LOG_ERR("threadPool %p fail to create thread. error code is %d\n", me, ret);
        me->aThreads[i] = 0;
      }
...
void ThreadPool_queue(ThreadPool *me, ThreadWork *work)
{
  ...
  if (QList_count(&me->qlWork) > me->nIdleThreads) {
    for (int j = 0; j < THREADPOOL_MIN_IDLE_THREADS; ++j) {
      (void)ThreadPool_createThread(me);
    }
  }
...
static int ThreadPool_createBaseThreads(ThreadPool *me)
{
  int ret = 0;

  vm_osal_mutex_lock(&me->qMtx);
  for (int i = 0; i < THREADPOOL_MIN_IDLE_THREADS; ++i) {
    for (int retry = 0; retry < THREADPOOL_MAX_RETRY; ++retry) {
      ret = ThreadPool_createThread(me);
      if (0 == ret) {
        break;
      } else {
        vm_osal_usleep(5000);
...
ThreadPool *ThreadPool_new(bool workForServer)
{
  ...
  // Create 4 threads ahead to make sure there are some threads to handle message.
  if (0 != ThreadPool_createBaseThreads(me)) {
    LOG_ERR("failed to create work thread\n");
    goto bail;
  }
