...
#endif
#define MINKD_OBJECT_RETRY_INTERVAL_MS 30
#define MINKD_OBJECT_RETRY_TIMEOUT_SEC (4 * 60) //4min

#define ACCEPT_THREADS_INIT_WAIT_TIME_MICROSEC 1000 //Retry time is set to 1ms.
...
int MinkCBContext::addMemObjToList(Object locobj, int32_t handle){
	...
	FdWrapper *ctx = FdWrapperFromObject(locobj);
	if (ctx != NULL) {
...
void MinkCBContext::WaitTillAcceptThreadsAreCreated()
{
	...
	//Wait until SIGUSR1 was unblocked.otherwise shutdown function will stuck
	while(maccept_thread_cnt != mcbthread_cnt)
	{
		ALOGV(" waiting %dus for accept thread to unblock SIGUSR1 maccept_thread_cnt  %d", ACCEPT_THREADS_INIT_WAIT_TIME_MICROSEC, maccept_thread_cnt);
		usleep(ACCEPT_THREADS_INIT_WAIT_TIME_MICROSEC);
	}
...
void *MinkCBContext::worker2(MinkCBContext* me)
{
	...
	pthread_mutex_lock(&mThreadReadyMutex);
	maccept_thread_cnt++;
	pthread_mutex_unlock(&mThreadReadyMutex);

	// When it fails to get the mShutDownMutex and gets the EBUSY code, it means that it is
	// under exiting process and it should wait for killing signal.
	if (pthread_rwlock_tryrdlock(&mShutdownMutex) == EBUSY) {
		goto wait_for_kill;
	}

	while (1) {

		ALOGV(" cbthread: tid %ld, send IOCTL_ACCEPT_REQ: has_response %d, txn_id %d, result %d, cbobj_id %d, op 0x%x, counts 0x%x maccept_thread_cnt %d,\n\n",
			tid, req.has_resp, req.txn_id, req.result, req.cbobj_id, req.op, req.counts, maccept_thread_cnt);
		pthread_rwlock_unlock(&mShutdownMutex);
		...
		if (ret) {
			...
			// It will wait for kill signal when the error code is not EINTR or EAGAIN.
			// It wont exit until getting the kill signal.
			pthread_rwlock_unlock(&mShutdownMutex);
			goto wait_for_kill;
		}
...
wait_for_kill:
	// Ensure the thread is waiting for killing signal.
	do {
		pause();
	} while (1);
	return NULL;
...
void MinkCBContext::shutdown()
{
	...
	pthread_rwlock_wrlock(&mShutdownMutex);
...
MinkCBContext::MinkCBContext(size_t cbthread_cnt,
			...
			mcbthread_cnt(cbthread_cnt), maccept_thread_cnt(0)
{
	pthread_attr_t attr;
	size_t i = 0;
	int ret = Object_OK;
	...
	pthread_rwlock_init(&mShutdownMutex, 0);
	pthread_mutex_init(&mThreadReadyMutex, 0);
...
{
	shutdown();
	pthread_mutex_destroy(&mLocObjTableMutex);
	pthread_rwlock_destroy(&mShutdownMutex);
	pthread_mutex_destroy(&mThreadReadyMutex);
	pthread_mutex_destroy(&mSyncMutex);
	pthread_cond_destroy(&mSyncCondVar);
...
static int64_t get_clock_time_in_ms(long offset_in_ms)
{
	struct timespec current_time;
	int64_t time_in_ms;

	if (clock_gettime(CLOCK_REALTIME, &current_time) != 0) {
		ALOGE("clock_gettime failed with errno=%d", errno);
		return -1;
	}

	time_in_ms = (int64_t)current_time.tv_sec * 1000 + current_time.tv_nsec / 1000000 + offset_in_ms;
			...
			ObjectCounts counts)
{
	...
	int64_t log_time = 0;
	int64_t retry_seconds_waited = 0;
	...
	log_time = get_clock_time_in_ms(0);
	do {
		memset(&req, 0, sizeof(struct smcinvoke_cmd_req));

		/* Marshal in before every ioctl to maintain input objects validity for the
		   corresponding ioctl. If input objects are not retained by target domain,
		   then all proxy objects are cleared in return path including
		   Minkdescriptor's object table entry. New object handle needs to be
		   generated and passed to maintain correctness. */
		...
		if (ret || req.result != Object_ERROR_BUSY)
			break;

		usleep(MINKD_OBJECT_RETRY_INTERVAL_MS * 1000); //sleep for 30ms.

		int64_t now = get_clock_time_in_ms(0);
		int64_t elapsed_time = (now - log_time) / 1000; // 1 seconds  = 1000 ms
		if (elapsed_time > retry_seconds_waited) {
			retry_seconds_waited = elapsed_time;
			ALOGV("Still retrying after %lld seconds...\n", retry_seconds_waited);
		}

		if (elapsed_time >= MINKD_OBJECT_RETRY_TIMEOUT_SEC) {
			ALOGE("Retry timeout reached %dsec. Breaking out of loop.", MINKD_OBJECT_RETRY_TIMEOUT_SEC);
			break;
		}

		memset(argptr, 0, ObjectCounts_total(counts) * sizeof(union smcinvoke_arg));
	} while (req.result == Object_ERROR_BUSY);
