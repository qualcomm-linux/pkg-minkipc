...
///
/// For more detail, refer to [go/mink](http://go/mink).
pub struct Object {
...
/// [`Object`] can be safely shared across threads as function pointers and contexts are valid.
///
/// Note this doesn't imply the underlying interface can skimp out on handling threading, this
/// simply means `&Object` can be shared across threads safely as invoke pointers are static and
/// the context wouldn't be freed until the ownership is removed.
unsafe impl Sync for Object {}
/// [`Object`] can be safely moved across threads as function pointers and contexts are valid.
...
impl Drop for Object {
    ...
    /// production.
    fn drop(&mut self) {
        let ret = match unsafe { self.invoke(OP_RELEASE, core::ptr::null_mut(), 0u32) } {
            0 => Ok(()),
            err => Err(err),
        };
        debug_assert!(
            ...
            behavior, if you believe this is a bug please submit
...
impl Clone for Object {
    ...
    fn clone(&self) -> Self {
        match unsafe { self.invoke(OP_RETAIN, core::ptr::null_mut(), 0u32) } {
            0 => Ok(()),
            err => Err(err),
        }
        .expect("Ref count value should be under usize::MAX");
