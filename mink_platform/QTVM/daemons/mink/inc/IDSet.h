...
static inline bool IDSet_testOrSet(IDSet *set, uint32_t id, bool modifySet)
{
    ...
    do {
        ...
        // We maintain a sorted (by group) list for efficiency reasons (ex.
        // subset
        // compares)

        C_FOR_ARRAY(ndx, set->entries) {
            ...
            *pe = insert;
            ...
            // End of the line, return true since we reached this only on insert
            // scenarios
            if (insert.group == 0) {
                return true;
            }
        ...
        if (!set->next && modifySet) {
            set->next = HEAP_ZALLOC_REC(IDSet);
        }
        set = set->next;
    } while (set);
...
static inline void IDSet_print(IDSet *set)
{
    int i = 0;
    size_t ndx;
    printf("------Start-----\n");
    do {
        printf("IDSet index: %d\n", i);
        C_FOR_ARRAY(ndx, set->entries) {
            IDSetEntry *pe = set->entries + ndx;
            printf("Entry ndx: %zu, Group = %X, Mask = %X\n", ndx, pe->group, pe->mask);
        }
        ++i;
        set = set->next;
    } while (set);
...
static inline bool IDSet_isSubSet(IDSet *set, IDSet *subset)
{
    ...
    do {
        ...
        C_FOR_ARRAY(subndx, subset->entries)
        {
            ...
            do {
                ...
                C_FOR_ARRAY(setndx, set->entries)
                {
...
static inline int32_t IDSet_dup(IDSet const *set, IDSet *dup)
{
    ...
    while (set->next) {
        tmp->next = HEAP_ZALLOC_TYPE(IDSet);
        T_CHECK_ERR(tmp->next != NULL, Object_ERROR_MEM);
